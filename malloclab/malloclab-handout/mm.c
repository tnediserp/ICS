...
static int travheap(int lineno)
{
    ...
    for(bp = heap_listp + LINK_HDRS_SIZE + DSIZE; 
    ...
    for(i=0; i < LINK_NUM; i++)
    {
        ...
        for(bp = bp0; bp; bp = NEXT_FREE_BLKP(bp))
        {
            ...
            if(index != i)
            {
                ...
                (size_t)bp);
                printf("size= %d, now in list %d, should in list %d\n",
                GET_SIZE(HDRP(bp)), i, index);
                exit(0);
            }
...
static void *up_insert_blkp(void *bp, int index)
{
    ...
    /* 注意：比较次数不超过MAX_TRAV次 */
    for(ptr = NEXT_FREE_BLKP(ptr0); 
    ptr != NULL && i < MAX_TRAV; 
    ptr = NEXT_FREE_BLKP(ptr), i++)
    {
