...
/* 给定bp, 求出它的left, right指针 */
#define LEFTP_DOMAIN(bp) ((char *)(bp))
#define RIGTP_DOMAIN(bp) ((char *)(bp) + WSIZE)
...
/* 寻找空闲块在空闲链表中的前驱和后继 */
#define LEFT_CHILD(bp)  ((char *) MEM_LOC(*(unsigned int *)(bp)))
#define RIGT_CHILD(bp)  ((char *) MEM_LOC(*((unsigned int *)(bp) + 1)))
...
static void *insert_blkp(void *bp, int index);
static void *parent(void *bp, int index);
...
/*
 * insert_blkp
 * 在第index个空闲链表中插入空闲块bp
 */
static void *insert_blkp(void *bp, int index)
{
    char *base = heap_listp + index*WSIZE;
    char *ptr = LEFT_CHILD(base);
    if(ptr == NULL)
    {
        PUT(LEFTP_DOMAIN(base), OFFSET(bp));
        PUT(LEFTP_DOMAIN(bp), UNSIGNED_MAX);
        PUT(RIGTP_DOMAIN(bp), UNSIGNED_MAX);
        return bp;
    }
    while(ptr != NULL)
    {
        base = ptr;
        if(GET_SIZE(HDRP(bp)) <= GET_SIZE(HDRP(base)))
        {
            ptr = LEFT_CHILD(base);
            if(ptr == NULL)
            {
        ...
        else
        {
            ...
            if(ptr == NULL)
            {
                PUT(RIGTP_DOMAIN(base), OFFSET(bp));
                break;
            }
        }
    }
...
static void *parent(void *bp, int index)
{
    //printf("in parent, index= %d\n", index);
    size_t size = GET_SIZE(HDRP(bp));
    char *root = LEFT_CHILD(heap_listp + index*WSIZE);
    char *ptr = root;
    if(root == bp)
    {
        return NULL;
...
static void remove_blkp(void *bp)
{
    int index = log_2(GET_SIZE(HDRP(bp)));
    
    //printf("in remove, bp= %lx, index= %d\n", (size_t)bp, index);
    char *base = heap_listp + index*WSIZE;
    char *tempptr;
    char *tempparent=NULL;
    char *oldparent = parent(bp, index);
    if(LEFT_CHILD(bp) == NULL)
        tempptr = RIGT_CHILD(bp);

    else
    {
        tempptr = LEFT_CHILD(bp);
        while(RIGT_CHILD(tempptr) != NULL)
        {
            tempparent = tempptr;
            tempptr = RIGT_CHILD(tempptr);
        }
        if(tempparent == NULL)
            PUT(LEFTP_DOMAIN(bp), OFFSET(LEFT_CHILD(tempptr)));

        else PUT(RIGTP_DOMAIN(tempparent), OFFSET(LEFT_CHILD(tempptr)));

        PUT(LEFTP_DOMAIN(tempptr), OFFSET(LEFT_CHILD(bp)));
        PUT(RIGTP_DOMAIN(tempptr), OFFSET(RIGT_CHILD(bp)));
    }

    if(oldparent == NULL)
        PUT(LEFTP_DOMAIN(base), OFFSET(tempptr));
    else if(LEFT_CHILD(oldparent) == bp)
    {
        PUT(LEFTP_DOMAIN(oldparent), OFFSET(tempptr));
    }
    else
    {
        PUT(RIGTP_DOMAIN(oldparent), OFFSET(tempptr));
    }
}

static void *upper_bound(size_t asize, int index)
{
    char *base = heap_listp + index*WSIZE;
    
    char *bp = LEFT_CHILD(base);
    //printf("in upper, base= %lx, bp= %lx\n", 
    //(size_t)base, (size_t)bp);
    char *ans = NULL;
    while(bp != NULL)
    {
