...
int mm_init(void) 
{
    ...
    PUT(heap_listp + LINK_HDRS_SIZE, UNSIGNED_MAX);                    /* Prologue header */ 
...
static void *coalesce(void *bp) 
{
    ...
    {      
        ...
        PUT(HDRP(prev_blkp), PACK(size, prev_prev_alloc << 1));
    ...
    {       
        ...
        size += GET_SIZE(HDRP(prev_blkp)) + 
            GET_SIZE(FTRP(next_blkp));
        PUT(HDRP(prev_blkp), PACK(size, prev_prev_alloc << 1));
        PUT(FTRP(next_blkp), PACK(size, prev_prev_alloc << 1));
        bp = prev_blkp;
...
static void *anti_coalesce(void *bp, size_t asize)
{
    ...
    {      
        ...
        if(size >= asize)
        {
            ...
            PUT(FTRP(bp), PACK(size, (prev_alloc << 1) | 1));
    ...
    {      
        ...
        {
            remove_blkp(prev_blkp);
            PUT(FTRP(bp), PACK(size, (prev_prev_alloc << 1) | 1));
    ...
    {       
        ...
        if(size >= asize)
        {
            ...
            char *next_next_blkp = NEXT_BLKP(next_blkp);
            PUT(HDRP(prev_blkp), PACK(size, (prev_prev_alloc << 1) | 1));
            PUT(FTRP(next_blkp), PACK(size, (prev_prev_alloc << 1) | 1));
            bp = prev_blkp;
            SET_PREV_ALLOC(HDRP(next_next_blkp));
        }
    }
