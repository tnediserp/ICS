...
int countTrailingZero(int x){
  int A,B,C,D,E,a,b,c,d,e,f,flag;
  A=0xffff;
  B=0xff;
  C=0xf;
  D=3;
...
int subOK(int x, int y) {
  ...
  z=~y+1;
  sum=x+z;
  flag=(x^y)&(x^sum); //溢出当且仅当x,y异号且x,sum异号
  flag>>=31;
  return !flag;
}
/* 
...
int isLessOrEqual(int x, int y) {
  int z,res1,res2,ans,flag;
  z=~x+1;
  res1=~(y+z);
  res2=x;
  flag=y^x; //y,x是否同号
  ans=(flag|res1)&(~flag|res2); //若异号，看x的正负；若同号，看y+z的正负
  return (ans>>31)&1;
}
/*
...
unsigned float_twice(unsigned uf) {
  ...
  b=uf&c;
  if(a==0) ans=b|(uf<<1);
  else if(a==255) ans=uf;
  else if(a==254) ans=b|subinf; //ans=(uf&(c>>8))|(255<<23);
  else ans=uf+d;
  return ans;
}
/* 
...
int float_f2i(unsigned uf) {
  ...
  f=M|A;
  if(E<=-2) ans=0;
  else if(E>=31) ans=1<<31;
  else if(E>=23) ans=f<<(E-23);
  else ans=f>>(23-E); //向0舍入
  ans=(ans^t)+s; //定符号
  return ans;
}
/* 
